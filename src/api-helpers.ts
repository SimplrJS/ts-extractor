...
export namespace ApiHelpers {
    ...
    export interface _ApiTypeBase {
        ApiTypeKind: TypeKinds;
        Text: string;
    }

    export interface _ApiTypeReference extends _ApiTypeBase {
        ApiTypeKind: TypeKinds.Basic | TypeKinds.TypeParameter;
        Name: string;
        ReferenceId: string | undefined;
        Generics: _ApiType[] | undefined;
    }

    export interface _ApiTypeUnionOrIntersection extends _ApiTypeBase {
        ApiTypeKind: TypeKinds.Union | TypeKinds.Intersection;
        Types: _ApiType[];
    }

    export interface _ApiTypeBasic extends _ApiTypeBase {
        ApiTypeKind: TypeKinds.Basic | TypeKinds.TypeParameter;
    }

    export type _ApiType = _ApiTypeUnionOrIntersection | _ApiTypeReference | _ApiTypeBasic;
    // tslint:enable:class-name

    export function TypeNodeToApiTypeDto(typeNode: ts.TypeNode, options: ApiItemOptions, self?: boolean): _ApiType {
        const typeChecker = options.Program.getTypeChecker();
        ...
        if (ts.isUnionTypeNode(typeNode) || ts.isIntersectionTypeNode(typeNode)) {
            ...
            const types = typeNode.types.map(x => TypeNodeToApiTypeDto(x, options));

            return {
                ApiTypeKind: kind,
                Types: types,
                Text: text
            } as _ApiTypeUnionOrIntersection;
        }

        // Resolve reference
        if (ts.isTypeReferenceNode(typeNode)) {
            const name = typeNode.typeName.getText();
            let generics: _ApiType[] | undefined;

            if (typeNode.typeArguments != null) {
                generics = typeNode.typeArguments.map(x => TypeNodeToApiTypeDto(x, options));
            }

            const symbol = self ? type.getSymbol() : type.aliasSymbol || type.getSymbol();
            let refenceId: string | undefined;
            if (symbol != null) {
                refenceId = resolveTypeItemReference(symbol, options).referenceId;
            }
